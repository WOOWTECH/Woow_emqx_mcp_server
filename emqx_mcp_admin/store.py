...
def env_from_tool_settings(tools: dict[str, Any]) -> dict[str, str]:
    ...
    return {
        "EMQX_MCP_DISABLED_CATEGORIES": ",".join(settings["disabled_categories"]),
        "EMQX_MCP_DISABLED_TOOLS": ",".join(settings["disabled_tools"]),
        "EMQX_MCP_DISABLED_OPERATIONS": json.dumps(settings["disabled_operations"]),
        "EMQX_MCP_READONLY": "true" if settings["readonly"] else "false",
    }
